...
class GradientDescent(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray, iteration: int) -> np.ndarray:
        ...
        :return: weight difference: np.ndarray
        """
        new_w = self.w - self.eta(iteration) * gradient
        delta = self.w - new_w
        self.w = new_w
        return delta
    ...
    def calc_gradient(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        :return: gradient: np.ndarray
        """
        return - 2 * np.dot(X.T, (y - np.dot(X, self.w))) / len(y)
...
class StochasticDescent(BaseDescent):
    ...
    def calc_gradient(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        batch = random.sample(range(len(y)), self.batch_size)
        return - 2 * np.dot(X[batch].T, (y[batch] - np.dot(X[batch], self.w))) / len(y[batch])
...
class Adagrad(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray, iteration: int) -> np.ndarray:
        ...
        self.g = self.g + pow(gradient, 2)
        new_w = self.w - self.eta(iteration) * gradient / np.sqrt(self.eps + self.g)
        delta = self.w - new_w
        self.w = new_w
        return delta
...
class LinearRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) :
        ...
        while(np.sum(self.descent.step(X, y, i - 1) ** 2) > self.tolerance and i < self.max_iter):
            self.calc_loss(X, y)
            i += 1
    ...
    def calc_loss(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        :param y: objects' target
        """
        self.loss_history.append(np.sum(pow((y - np.dot(X, self.descent.w)), 2)) / len(y))
        #self.loss_history.append(- 2 * np.dot(X.T, (y - np.dot(X, self.descent.w))) / len(y))
